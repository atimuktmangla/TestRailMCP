...
**Python:** **3.11** is the minimum supported version. **All newer CPython 3.x releases** (3.12, 3.13, and future 3.x) are supported — same as `requires-python >=3.11` in the package metadata.
...
1. Install **Python 3.11 or any newer 3.x** from [python.org](https://www.python.org/downloads/) (Windows: enable **Add Python to PATH**; the full installer is more reliable than depending only on the Store “python” alias).
...
| **`INSTALL.cmd`**: “Python not found” but `python --version` works elsewhere | **Command Prompt** may not have the same **PATH** as PowerShell or your IDE. On Windows, the first `python` on PATH is sometimes the **Microsoft Store stub** (`…\WindowsApps\python.exe`), which fails in **cmd**; the script prefers the **`py`** launcher when present. Install from [python.org](https://www.python.org/downloads/) with **Add python.exe to PATH**, or run `py -3 --version` in **cmd**, then re-run **`INSTALL.cmd`**. Manual install: `py -3 -m venv .venv` (or `python -m venv .venv`), activate, `pip install wheel\*.whl`. |
...
- **[WIKI_CLIENT.md](WIKI_CLIENT.md)** — self-contained page for **Confluence / internal wiki**: install, Cursor JSON, **tool list**, and **example prompts** (paste as-is; no other files required).
